...
# 2D score plot
PlotPCA2DScore <- function(imgName, format="png", dpi=72, width=NA, pcx, pcy, reg = 0.95, show=1, grey.scale = 0, posneg = ""){
    ...
    if(dataSet$cls.type == "disc"){
        ...
        legend("topleft", legend = posneg, text.font = 2, bty = "n"); # Yang: text.font=2 : bold; 3: italic
    }else{
...
# score plot
PlotPLS2DScore<-function(imgName, format="png", dpi=72, width=NA, inx1, inx2, reg=0.95, show=1, grey.scale=0, use.sparse=FALSE, posneg = ""){
...
PlotPLS.Imp<-function(imgName, format="png", dpi=72, width=NA, type, feat.nm, feat.num, color.BW=FALSE, posneg=""){
    imgName = paste(imgName, "dpi", dpi, ".", format, sep="");
    if(is.na(width)){
        w <- 8;
    }else if(width == 0){
    ...
    if(type=="vip"){
        ...
        PlotImpVar(vips, "VIP scores", feat.num, color.BW, posneg);
    }else{
        analSet$plsda$imp.type<<-"coef";
        data<-analSet$plsda$coef.mat[,feat.nm];
        PlotImpVar(data, "Coefficients", feat.num, color.BW, posneg);
    }
...
# BHan: added bgcolor parameter for B/W color
PlotImpVar <- function(imp.vec, xlbl, feat.num=15, color.BW=FALSE, posneg=""){
    ...
    pls_permcol <- colorRampPalette(brewer.pal(11, colorpalette))(50) 
    # Yang: change 25 to 11 in order to void the warning msg: In brewer.pal(25, colorpalette): n too large, allowed maximum for palette RdYlGn is 11
    if(color.BW) col <- rev(col);
    ...
    text(round(imp.vec[1], digits = 1), endy+shifty/1.25, posneg, font = 2);  # Yang: print POS/NEG, font = 2 bold
...
# Plot plsda classification performance using different components
PlotPLS.Permutation<-function(imgName, format="png", dpi=72, width=NA, posneg=""){
    ...
    text(2, h, posneg, xpd=T, font = 2);  # Yang: add POS or NEG
    dev.off();
}
