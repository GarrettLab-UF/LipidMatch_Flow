...
Ttests.Anal<-function(nonpar=F, threshp=0.05, paired=FALSE, equal.var=TRUE){
        ...
        msg <- NULL;
        while(sum(inx.imp)==0){
            threshp <- threshp + 0.05;
            inx.imp <- p.value <= threshp;
            msg <- paste(c("No sig. features were identified. The threshold is adjusted to", threshp), collapse=" ");
        }
        msg <- paste(c(msg, "A total of", sum(inx.imp), "significant features were found."), collapse=" ");
