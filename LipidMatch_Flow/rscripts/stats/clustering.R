...
PlotSubHeatMap <- function(imgName, format="png", dpi=72, width=NA, dataOpt, scaleOpt, smplDist, clstDist, palette, method.nm, top.num, viewOpt, rowV=T, colV=T, border=T, posneg=""){
    ...
    PlotHeatMap(imgName, format, dpi, width, dataOpt, scaleOpt, smplDist, clstDist, palette, viewOpt, rowV, colV, var.inx, border, posneg);
}


PlotHeatMap<-function(imgName, format="png", dpi=72, width=NA, dataOpt, scaleOpt, smplDist, clstDist, palette, viewOpt="detail", rowV=T, colV=T, var.inx=NA, border=T, posneg=""){
   ...
   if(viewOpt == "overview"){
    ...
    if(dataSet$cls.type == "disc"){
        ...
        pheatmap(t(hc.dat), 
            ...
            main = posneg    # Yang: print POS/NEG
            );
    }else{
