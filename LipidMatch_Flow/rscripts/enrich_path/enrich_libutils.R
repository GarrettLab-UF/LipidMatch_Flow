...
CreateMsetLibFromCSV<-function(){
    fileName <- paste("../../libs/msets/mset_", libname, ".csv", sep="");

    # print(paste("loading library", fileName));
    current.msetlib <<- read.csv(fileName, as.is=T, header=T);

    # format references to html links with informative name
    ref.len <- nrow(current.msetlib);
    all.refs <- vector(mode = "character", length = ref.len);
    for(i in 1:ref.len){
        refs <- strsplit(current.msetlib$reference[i], "; ")[[1]];
        nms <- vector(mode = "character", length = length(refs));

        for(m in 1:length(refs)){
            if(length(grep("smpdb", refs[m]))>0){
                nm <- "SMPDB";
            }else if(length(grep("hmdb", refs[m]))>0){
                nm <- "HMDB";
            }else if(length(grep("metagene", refs[m]))>0){
                nm <- "MIC";
            }else if(length(grep("pubmed", refs[m]))>0){
                nm <- "PubMed";
            }else if(length(grep("doi", refs[m]))>0){
                nm <- "DOI";
            }else{
                # cat("Weird, mset without reference!", i, "\n");
                nm <- NA;
            }
            nms[m]<-nm;
        }
        if(is.na(nms)){
            all.refs[i] <- "Under Review";
        }else{
            all.refs[i]<-paste("<a href=\"", refs, "\" target=\"_blank\"><u>", nms, "</u></a>", sep="",  collapse="\n");
        }
    }
    current.msetlib$reference <<- all.refs;
    current.msetlib$lib.name <<- libname;
    save(current.msetlib, file=paste(libname, ".rda", sep=""));
}
...
Setup.UserMsetLibData<-function(filePath){
    ...
    dat<-try(read.table(inFile, sep=",", as.is=T, header=F));
    close(inFile);
    libCheck.msg <- NULL;
...
Get.ConcRef<-function(cmpd.nm){
    if(is.null(conc.db)){
        conc.db <<- read.csv("../../libs/cmpd_conc.csv", as.is=T, header=T);
    }
...
LoadSmpLib<-function(){
    paths <- read.csv("../../libs/smp_path.csv", as.is=T, header=T);
...
SetKEGG.PathLib<-function(kegg.rda){
       ...
       # load(path.dir); # load to the current env
       # attach(path.dir, warn.conflicts = FALSE); # attach to the parent env
